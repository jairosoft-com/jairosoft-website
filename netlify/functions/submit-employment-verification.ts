 ...
 */
async function createContact(formData: EmploymentVerificationFormData, locationId: string, apiKey: string): Promise<GoHighLevelContact> {
  const contactData = {
...
/**
 * Add a note to a contact with employment verification details
 */
async function addVerificationNote(contactId: string, formData: EmploymentVerificationFormData, fileUrl: string | null, apiKey: string): Promise<void> {
  const noteContent = `
...
async function createOpportunity(
  ...
  // Add custom field for verification files if we have a file URL
  if (fileUrl) {
    opportunityData.customFields = [
      {
        id: 'employment_verification_files', // This may need to be updated with actual custom field ID
...
export const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    ...
    // Parse request body
    const formData: EmploymentVerificationFormData = JSON.parse(event.body || '{}');

    // Validate required fields
    if (!formData.firstName || !formData.lastName || !formData.email ||
        !formData.position || !formData.specificRequest ||
    ...
    // Add note to contact with verification details and file URL
    ...
    console.error('Error in submit-employment-verification function:', error);
    return {
      statusCode: 500,
      body: JSON.stringify({
